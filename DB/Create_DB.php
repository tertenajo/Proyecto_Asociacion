...
require_once("../includes/initializer.php");
    ...
    if(mysqli_query($conexion,$crearbd))
    {
        ...
        $usuarios="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."usuarios(
                    ...
                    CONSTRAINT FK_Nivel_Formativo FOREIGN KEY (nivel_formativo) REFERENCES ".PREF_TABLA."nivel_formativo(id_formativo) ON UPDATE CASCADE,
                    CONSTRAINT FK_Situacion_Laboral FOREIGN KEY (situacion_laboral) REFERENCES ".PREF_TABLA."situacion_laboral(id_laboral)ON UPDATE CASCADE);";
        $datos_abjuntos="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."datos_abjuntos(
                        id_adjunto INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        id_usuario INT(5) NOT NULL,
                        descripcion VARCHAR(200) NOT NULL,
                        CONSTRAINT FK_ID_Usuario FOREIGN KEY (id_usuario) REFERENCES ".PREF_TABLA."usuarios(id_usuario) ON UPDATE CASCADE ON DELETE CASCADE);";
        $nivel_formativo="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."nivel_formativo(
                        id_formativo INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        nombre VARCHAR(150) NOT NULL,
                        descripcion VARCHAR(200) NOT NULL);";
        $situacion_laboral="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."situacion_laboral(
                        id_laboral INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        nombre VARCHAR(150) NOT NULL,
                        descripcion VARCHAR(200) NOT NULL);";
        $prestacion_social="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."prestacion_social(
                        id_prestacion INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        nombre VARCHAR(150) NOT NULL);";
        $prestacion_recibida="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."prestacion_recibida(
                        id_usuario INT(5) NOT NULL,
                        id_prestacion INT(5) NOT NULL,
                        nombre VARCHAR(150) NOT NULL,
                        descripcion VARCHAR(200) NOT NULL,
                        PRIMARY KEY (id_usuario,id_prestacion),
                        CONSTRAINT FK_Usuarios FOREIGN KEY (id_usuario) REFERENCES ".PREF_TABLA."usuarios(id_usuario) ON UPDATE CASCADE ON DELETE CASCADE,
                        CONSTRAINT FK_ID_Prestacion FOREIGN KEY (id_prestacion) REFERENCES ".PREF_TABLA."prestacion_social(id_prestacion) ON UPDATE CASCADE);";
        $profesionales="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."profesionales(
        ...
        $perfiles_profesionales="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."perfiles_profesionales(
                        id_perfil INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        tipo VARCHAR(150) NOT NULL);";
        $proyectos="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."proyectos(
                        id_proyecto INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        nombre VARCHAR(150) NOT NULL);";
        $funcion_proyecto="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."funcion_proyecto(
                        ...
                        CONSTRAINT FK_Profesional FOREIGN KEY (id_profesional) REFERENCES ".PREF_TABLA."profesionales(id_profesional) ON UPDATE CASCADE ON DELETE CASCADE,
                        CONSTRAINT FK_Proyectos FOREIGN KEY (id_proyecto) REFERENCES ".PREF_TABLA."proyectos(id_proyecto) ON UPDATE CASCADE ON DELETE CASCADE,
                        CONSTRAINT FK_Perfil_Profesional FOREIGN KEY (id_perfil) REFERENCES ".PREF_TABLA."perfiles_profesionales(id_perfil) ON UPDATE CASCADE ON DELETE CASCADE);";
        $tipo_atencion="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."tipo_atencion(
                        id_atencion INT(5) NOT NULL AUTO_INCREMENT PRIMARY KEY,
                        nombre VARCHAR(150) NOT NULL);";
        $sesiones="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."sesiones(
                        ...
                        descripcion VARCHAR(200) NOT NULL,
                        CONSTRAINT FK_Atencion FOREIGN KEY (tipo_atencion) REFERENCES ".PREF_TABLA."tipo_atencion(id_atencion) ON UPDATE CASCADE,
                        CONSTRAINT FK_Proyecto FOREIGN KEY (proyecto) REFERENCES ".PREF_TABLA."proyectos(id_proyecto) ON UPDATE CASCADE);";
        $sesiones_impartidas="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."sesiones_impartidas(
                        id_profesional INT(5) NOT NULL,
                        id_sesion INT(5) NOT NULL,
                        PRIMARY KEY (id_profesional,id_sesion),
                        CONSTRAINT FK_ProfesionalI FOREIGN KEY (id_profesional) REFERENCES ".PREF_TABLA."profesionales(id_profesional) ON UPDATE CASCADE,
                        CONSTRAINT FK_SesionI FOREIGN KEY (id_sesion) REFERENCES ".PREF_TABLA."sesiones(id_sesion) ON UPDATE CASCADE);";
        $sesiones_individuales="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."sesiones_individuales(
                        id_sesion INT(5) NOT NULL PRIMARY KEY,
                        id_usuario INT(5) NOT NULL,                        
                        CONSTRAINT FK_UsuarioI FOREIGN KEY (id_usuario) REFERENCES ".PREF_TABLA."usuarios(id_usuario) ON UPDATE CASCADE ON DELETE CASCADE,
                        CONSTRAINT FK_SesionesI FOREIGN KEY (id_sesion) REFERENCES ".PREF_TABLA."sesiones(id_sesion) ON UPDATE CASCADE ON DELETE CASCADE);";
        $sesiones_grupales="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."sesiones_grupales(
                        id_sesion INT(5) NOT NULL PRIMARY KEY,
                        CONSTRAINT FK_SesionesG FOREIGN KEY (id_sesion) REFERENCES ".PREF_TABLA."sesiones(id_sesion) ON UPDATE CASCADE ON DELETE CASCADE);";
        $participacion_grupal="CREATE TABLE IF NOT EXISTS ".PREF_TABLA."participacion_grupal(
                        id_sesion INT(5) NOT NULL,
                        id_usuario INT(5) NOT NULL,
                        PRIMARY KEY(id_sesion,id_usuario),                        
                        CONSTRAINT FK_UsuarioGR FOREIGN KEY (id_usuario) REFERENCES ".PREF_TABLA."usuarios(id_usuario) ON UPDATE CASCADE ON DELETE CASCADE,
                        CONSTRAINT FK_SesionesGR FOREIGN KEY (id_sesion) REFERENCES ".PREF_TABLA."sesiones_grupales(id_sesion) ON UPDATE CASCADE ON DELETE CASCADE);";
